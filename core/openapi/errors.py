...
ERRORS = [
    ...
    _e("PICKUP_PHOTOS_REQUIRED", 409, G_ITEMS,
       "The order was booked with `pickup_photo` (`order` or `per_item`) and the driver hasn't taken all of those photos yet, so the delivery can't start.",
       "Have the driver take them (`POST /driver/trips/{id}/pickup-photo`), then retry.",
       "Take the pickup photos before starting the delivery."),
    _e("DELIVERY_PHOTOS_REQUIRED", 409, G_ITEMS,
       "The order was booked with `delivery_photo` (`order` or `per_item`) and the driver hasn't taken all of those photos at the drop yet, so it can't be paid for or completed.",
       "Have the driver take them (`POST /driver/trips/{id}/delivery-photo`), then retry.",
       "Take the delivery photos before finishing the delivery."),
    _e("DELIVERY_PHOTO_NOT_REQUESTED", 409, G_ITEMS,
       "The driver sent a delivery photo for an order booked with `delivery_photo: none`.",
       "Only orders booked with `delivery_photo: order` or `per_item` take delivery photos.",
       "This order doesn't need delivery photos."),
    _e("PICKUP_PHOTO_NOT_REQUESTED", 409, G_ITEMS,
       "The driver sent a pickup photo for an order booked with `pickup_photo: none`.",
       "Only orders booked with `pickup_photo: order` or `per_item` take pickup photos.",
       "This order doesn't need pickup photos."),
