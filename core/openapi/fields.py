...
FIELDS = {
    ...
    "TripCreate": {
        ...
        "notes": "A note for the driver about the whole order - \"Call before arriving\", \"Use gate 2\". Shown on the order details screen. Optional, up to 500 characters.",
        ...
        "pickup_photo": "Photos the driver must take **with the phone's camera** at the pickup before the delivery can start: `none` (default), `order` - one photo of the whole package, or `per_item` - one photo of every item (requires `items`). Kept as proof of what left the pickup.",
        "delivery_otp": "Prepaid trips only: `true` texts the customer (`drop.contact_phone`, then required) a delivery OTP at the drop which the driver must enter to complete - proof the right person got the goods. Cash-on-delivery trips always use one. Default `false`.",
        "delivery_photo": "The same at the **drop**: camera photos the driver must take once the delivery is under way, before it can be paid for or completed - `none` (default), `order` or `per_item` (requires `items`). Kept as proof of delivery.",
        "items": f"The goods being delivered, one entry per line (at most 100). Optional; required if `verify_items` is `true`.",
        "bonus_fare": "An extra flat amount for this trip specifically - e.g. compensation for unloading heavy goods - paid to the driver on top of the fare card. Decimal, 0 or more. Never charged to the customer; does not affect `total_fare`. Optional, omit or send `0` for none.",
    },
    "TripItemInput": {
    ...
    "TripItem": {
        ...
        "pickup_photo_url": "The photo the driver took of this item at the pickup (orders with `pickup_photo: per_item`), or `null`.",
        "delivery_photo_url": "The photo the driver took of this item at the drop (orders with `delivery_photo: per_item`), or `null`.",
    ...
    "Trip": {
        ...
        "order_number": "Our order number, shown to the driver: `OD` + the booking date (`YYYYMMDD`) + `000` + a running number, e.g. `OD20260925000123`. Assigned at booking, never changes. (A trip with several drops would add `_01`, `_02`... per delivery; every trip has one drop today.)",
        "reference_id": "Your own order number, exactly as you sent it (empty string if you sent none).",
        "notes": "Your note for the driver about the whole order, or empty.",
        ...
        "bonus_fare": "An extra flat amount for this trip specifically (e.g. for unloading heavy goods), as sent when it was booked, or `null` if none. Decimal string. Paid to the driver in full, on top of their usual share of `total_fare`; never charged to the customer.",
        ...
        "pickup_photo": "Camera photos the driver owes at the pickup before starting: `none`, `order` (one of the whole package - `pickup_photo_url`) or `per_item` (one per item - `items[].pickup_photo_url`).",
        "pickup_photo_url": "The photo of the whole order the driver took at the pickup (`pickup_photo: order`), or `null`.",
        "delivery_otp": "Whether completing needs the customer's delivery OTP on a prepaid trip (cash-on-delivery trips always do).",
        "delivery_photo": "Camera photos the driver owes at the drop before payment/completion: `none`, `order` (`delivery_photo_url`) or `per_item` (`items[].delivery_photo_url`).",
        "delivery_photo_url": "The photo of the whole order the driver took at the drop (`delivery_photo: order`), or `null`.",
...
FIELDS.update({
    ...
    "DriverOtpVerify": {
        ...
        "otp": "The 4-digit code from the SMS (valid 5 minutes, single use). Required.",
    },
    ...
    "TripComplete": {"otp": "The 4-digit delivery OTP the customer was texted. **Required for a cash-on-delivery trip**, ignored for prepaid."},
    "TripItemVerify": {
    ...
    "TripPickupPhoto": {
        "photo": "The picture, taken with the phone's camera at the pickup (the app stamps it with the location and time). Image, up to 10 MB. Replaces any earlier photo for the same slot.",
        "item_id": "Which item the photo shows - one of the trip's `items[].id`. **Required when the order's `pickup_photo` (or, for delivery photos, `delivery_photo`) is `per_item`**; leave out for `order`.",
    },
})
